...
const LaborRegisterForm = () => {
  ...
  const { handleNextStep, handlePrevStep } = useMultiStep({
    activeStep,
    setActiveStep,
    childrenLength: LABOR_TOTAL_FORMS + 2,
  });

  useScrollToTopEffect(activeStep);

  const validationSchema = MAP_LABOR_FORM_TO_VALIDATION?.[activeStep];

  const {
    mutate: registerLabor,
    isLoading,
    isError,
    error_message,
  } = useRegisterLabor({
  ...
  const handleSubmit = (values: ValuesType) => {
    if (activeStep + 1 !== LABOR_TOTAL_FORMS) {
      handleNextStep();
      return;
    }
    registerLabor(values);
  };
      ...
      >
        {(formik) => (
          <Form autoComplete="off">
            ...
            >
              <BasicInfoForm role={ROLE.LABOR} isLoading={isLoading} />
...
export default LaborRegisterForm;
